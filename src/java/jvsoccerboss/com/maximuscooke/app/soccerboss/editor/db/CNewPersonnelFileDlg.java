...
public class CNewPersonnelFileDlg extends CDlgBase
{
	...
	private CJLabel mNameLbl = new CJLabel("Forename :");
	private CJLabel mExtLbl = new CJLabel("Surname :");
	...
	public CNewPersonnelFileDlg()
	{
		this(null, null);
	}
	
	public CNewPersonnelFileDlg(String dir, String fName)
	{
		super("Personnel File", "Personnel File Details", "Add new personnel file name details", 900, 560, CResource.getResourceAsImageIcon(CSoccerBoss.RES_DIR + "ac0051-48.png"), true);
	...
	private void load(String dir, String fName)
	{
		...
		{			
			...
			try
			{
				...
				CRange<Long> range = tf.findMarkerRange(CSoccerBoss.FORENAME_MARKER);
				...
				range = tf.findMarkerRange(CSoccerBoss.SURNAME_MARKER);
			...
			{
				JOptionPane.showMessageDialog(this, e.getMessage(), "Error Opening Personnel File", JOptionPane.ERROR_MESSAGE);
			}
		}
	}
	...
	private boolean save(String dir, String fName)
	{
		if (dir != null && fName != null)
		{
			if (CFile.exists(dir, fName) && JOptionPane.showConfirmDialog(this, String.format("File <%s> already exists, overwrite (Y/N)", fName), "Save Personnel File", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.NO_OPTION)
			{				
				return false;
			}
			...
			tf.addLine(CSoccerBoss.FORENAME_MARKER);

			this.mNameMap.values().forEach( ln -> tf.addLine(ln) );
			
			tf.addLine(CSoccerBoss.FORENAME_MARKER);
			
			tf.addLine(CSoccerBoss.SURNAME_MARKER);
...
//			
//			FileNameExtensionFilter filter = new FileNameExtensionFilter("Personnel file", CSoccerBoss.CLUB_EXT);
//			
//			fc.addChoosableFileFilter(filter);SETUP_DIRb
//			
...
//				
//				this.mFilename = f.getName() + "." + CSoccerBoss.PERSONNEL_EXT;
//			}	
//		}
//		
